  ...
  <div>
    <el-container>
      ...
      <el-main style="z-index: 999;display: flex;justify-content: center">
        <div class="adjust_width publish_width publish_main">
          <el-container>
            <el-aside style="width:150px" class="displayNav">
              <Nav :activate_nav="'发表文章'"/>
            </el-aside>
            <el-main>
              <div class="title">
                <el-row type="flex" class="row-bg" justify="space-between">
                  <el-col :span="8">
                    <el-row class="latexTips_input">
                      <el-col :span="16">
                        <el-input
                          placeholder="请输入标题"
                      ...
                      <el-col :span="8">
                        <span>&nbsp;&nbsp;&nbsp;&nbsp;</span><span id="saveTime" style="color: #c9c9c9">未</span>
                        &nbsp;<span style="color: #c9c9c9">保存</span>
                      </el-col>
                    </el-row>
                  </el-col>
                  <el-col :span="12">
              ...
              <el-row type="flex" class="row-bg font" justify="space-between">
                <el-col :span="7" class="adjust_save">
                  <el-button @click="saveMavonToDraft">保存到草稿</el-button>
                  <el-button @click="submitMavon" v-loading.fullscreen.lock="fullscreenLoading" type="success">发表文章</el-button>
                </el-col>

                <el-col :span="12" class="latexTips_input">
                  <el-col :span="6" class="adjust_tip">latex 数学符号提示</el-col>
                ...
                <el-col :span="5" class="adjust_gap">
<!--                  <el-button type="primary" plain>上传 md 文件</el-button>-->
                </el-col>
              </el-row>
...
<script>
  ...
  import {addArticle, addDraft, deleteImg, getRePublishArticle, reEditorSubmit, uploadImg} from "../../network/publish";
  import {getMDs} from "../../network/articleDetails";
  import axios from "axios";
  import {getReDraftInfo} from "../../network/draft";
  ...
  let timer = null;
    ...
    export default {
      ...
      created() {
        ...
        if(localStorage.getItem("MDCont")!==null || localStorage.getItem("MDTitle")!==null){
          this.title=localStorage.getItem("MDTitle") || ""
          this.content=localStorage.getItem("MDCont")|| ""
          this.scope=localStorage.getItem("MDScope")|| ""
          if(localStorage.getItem("MDTag")!=='undefined' && localStorage.getItem("MDTag")!==undefined){
            this.dynamicTags = localStorage.getItem("MDTag").split(",") || ['随笔']
          }
        }

        let status = localStorage.getItem("editorStatus") || ""
        if(status==="reEditor"){
          let needLoad = localStorage.getItem("needLoad") || "";
          if(needLoad!=="false"){
            let aid = localStorage.getItem("editorID") || ""
            if(aid.length>0){
              getRePublishArticle(parseInt(aid)).then(res =>{
                this.title = res.data.data.title
                this.dynamicTags = res.data.data.tags.split(',')
                if(res.data.data.scope ===1 ){
                  this.scope = "公开"
                }else {
                  this.scope = res.data.data.scope === 2 ?"私密":"仅粉丝可见"
                }
                this.reEditorDesc=res.data.data.content

                axios.defaults.headers = {}
                getMDs(parseInt(res.data.data.authorId),res.data.data.content).then(res=>{
                  axios.defaults.headers = {
                  ...
                  this.alertUrl()
                  this.saveMavon()
                  localStorage.setItem("needLoad","false")
                })
              }).catch(()=>{
                axios.defaults.headers = {
                  "Content-Type": "application/x-www-form-urlencoded;charset=UTF-8",
        ...
        else if(status==="reDraft"){
          let needLoad = localStorage.getItem("needLoad") || "";
          if(needLoad!=="false"){
            let aid = localStorage.getItem("draftID") || ""
            if(aid.length>0){
              getReDraftInfo(parseInt(aid)).then(res=>{
      ...
      methods:{
        submitMavon(){
          if(this.hasLogin()){
            if(this.title.trim()==='' || this.title.length>50){
              this.$message("标题不能为空且长度不能超过50")
        ...
        reEditorSubmit(){
          ...
          if(this.scope!=="私密" && this.scope!=="仅粉丝可见"){
            this.scope="公开"
          }

          reEditorSubmit(this.title,this.scope,this.content,this.dynamicTags,this.reEditorDesc,localStorage.getItem("editorID"))
            .then(res => {
              if(res.data.code === 20000){
                this.title=''
                this.content=''
                ...
                if(localStorage.getItem("MDTitle")!==null){
                  ...
                  localStorage.removeItem("MDTag")
                }

                loading.close();

                localStorage.setItem("editorStatus","")
        ...
        publishArticle() {
          ...
          addArticle(this.title,this.scope,this.content,this.dynamicTags)
            .then(res => {
              if(res.data.code === 20000){
                this.title=''
                this.content=''
        ...
        $imgAdd(pos, $file) {
          if(this.hasLogin()){
            this.base64ThumbImage($file, 840, 840,pos);
          }else{
            this.content = this.content.replace(new RegExp(("!\\[("+$file.name+")\\]\\(("+pos+")\\)"), 'g'),"")
            document.getElementById("login").style.display = "block"
            document.getElementsByClassName("v-modal")[0].style.display = "block"
            this.$message.info("请先登录")
          }
        ...
        base64ThumbImage(elementObj,maxWidth,maxHeight,pos){
          let _this = this
          let current=""
          if (typeof (FileReader) === 'undefined') {
            ...
            try {
              ...
              img.onload = function () {
                ...
                uploadImg(compressionImg).then((res)=>{
        ...
        alertUrl(){
          let user =  JSON.parse(localStorage.getItem("nbUser"))
          let serverPath = 'http://nblog.org.cn/images/'
          let _this = this;
          // 匹配博文中所有图片，[]兜底防止匹配不到
          let fakeUrls = _this.content.match(/!\[(.*?)\]\((.*?)\)/g) || [];
          // backUrl替换为realUrl，以正确显示博文图片
          fakeUrls.forEach(function(item) {
            let url = item.substring(item.lastIndexOf("(")+1,item.indexOf(")"))
            _this.$refs.md.$imgUpdateByUrl(url, serverPath + user.id + "/" + url +".png");
          });
        },
        fullScreen(){
          if(!this.fullS){
            ...
            document.getElementById("editor").style.maxHeight="99vh";
            this.fullS=true
          }else{
            this.$message('退出全屏编辑模式');
            document.getElementById("editor").style.minHeight="71vh";
            this.fullS=false
          }
        },
        saveMavon(){
          ...
          localStorage.setItem("MDTag",this.dynamicTags)
          ...
          var myDate = new Date()
          document.getElementById("saveTime").innerHTML = myDate.getHours()+":"+myDate.getMinutes()+":"+myDate.getSeconds()

        },
        saveMavonToDraft(key){
          let _this = this
          if(this.hasLogin()){
            if(this.title.trim()==='' || this.title.length>60){
              this.$message("标题不能为空且长度不能超过60")
            }else{
              const loading = this.$loading({
                lock: true,
                text: 'Loading',
              ...
              addDraft(this.title,this.scope,this.content,this.dynamicTags).then(res=>{
                if(res.data.code === 20000){
                  _this.title=''
                  _this.content=''
                  _this.dynamicTags=['随笔']
                  _this.scope='公开'
      ...
      mounted() {
        ...
        timer = setInterval(function(){
          if(_this.content.trim()!=="" || _this.title.trim()!==""){
            localStorage.setItem("MDCont",_this.content)
            localStorage.setItem("MDTitle",_this.title)
            localStorage.setItem("MDScope",_this.scope)
            localStorage.setItem("MDTag",this.dynamicTags)
            document.getElementById("saveTime").innerHTML = new Date().getHours()+":"+new Date().getMinutes()+":"+new Date().getSeconds()
          }
        },1000*30);
      },destroyed() {
        clearInterval(timer)
